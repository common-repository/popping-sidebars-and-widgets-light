			...
			if( typeof( json_object[ json_row ].columns ) == 'object' ){
				for( var cC = 0; cC < json_object[ json_row ].columns.length; cC++ ){
					...
					for( var cS = 0; cS < json_object[ json_row ].columns[ cC ].shortcodes.length; cS++){
						if( typeof( otw_overlay_shortcode_component.shortcodes[ json_object[ json_row ].columns[ cC ].shortcodes[ cS ].shortcode_type ] ) != 'undefined' ){
...
otw_overlay_grid_manager_object.prototype.close_dropdowns = function(){
	...
	if( typeof( otw_overlay_shortcode_component ) != 'undefined' ){
		if( otw_overlay_shortcode_component.dropdown_menu ){
			otw_overlay_shortcode_component.dropdown_menu.hide();
		};
	};
...
otw_overlay_grid_manager_object.prototype.show_add_column_menu = function( append_to, row_id ){
	...
	if( link.parent().hasClass('otw-row-nocolumn') ){
	...
	for( var cR = 0; cR < this.rows.length; cR++){
		...
		for( var cC = 0; cC < this.rows[ cR ].columns.length; cC++){
			...
			for( var cS = 0; cS < this.rows[ cR ].columns[ cC ].shortcodes.length; cS++ ){
				...
				var image = '<div class="otw-shortcode-title-preview">' + otw_overlay_shortcode_component.shortcodes[this.rows[ cR ].columns[ cC ].shortcodes[ cS ].shortcode_type ].title + '</div>';
...
otw_overlay_grid_manager_object.prototype.init_controls = function(){
	...
	this.preview_container.find( 'a.otw-column-control-add' ).on( 'click',  function( event ){
		...
		if( typeof( otw_overlay_shortcode_component ) != 'object' ){
			...
			otw_overlay_shortcode_component.open_drowpdown_menu( this );
			
			otw_overlay_shortcode_component.insert_code = function( shortcode_object ){
...
function otw_overlay_grid_manager_row(){

	this.columns = new Array();
};

function otw_overlay_grid_manager_column( rows, from_rows, mobile_rows, mobile_from_rows ){
...
function otw_overlay_grid_manager_shortcode( settings ){
...
otw_overlay_grid_manager_shortcode.prototype.html_code = function( scode_object_name, row_id, column_id, shortcode_id, parent, total_shortcodes ){
			...
			html = html + '<div class="otw-column-shortcode-content">';
				html = html + '<div class="otw-shortcode-controls">';
					...
					if( typeof( this.iname ) != 'undefined' ){
						html = html + '<div class="otw-shortcode-control-move"><span>' + otw_overlay_shortcode_component.shortcodes[ this.shortcode_type ].title  + ' - ' + this.iname + '</span>';
					}else{
						html = html + '<div class="otw-shortcode-control-move"><span>' + otw_overlay_shortcode_component.shortcodes[ this.shortcode_type ].title + '</span>';
					}
