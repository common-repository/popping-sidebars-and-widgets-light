				...
				foreach( $rows as $row ){
					...
					if( $cSize = count( $row->columns )){
						...
						foreach( $row->columns as $column ){
							...
							$grid_content .= '[otw_shortcode_grid_column rows="'.esc_attr( $column->rows ).'" from_rows="'.esc_attr( $column->from_rows ).'" mobile_rows="'.esc_attr( $column->mobile_rows ).'" mobile_from_rows="'.esc_attr( $column->mobile_from_rows ).'"'.$is_last.']';
