...
otw_shortcode_object.prototype.init_dropdown_actions = function(){
	...
	jQuery( 'body' ).on( 'click',  function( event ){
		...
		if( jQuery( event.target ).parents( '.mce-btn' ).attr( 'data-otwkey' ) == otw_overlay_shortcode_component.tinymce_button_key ){
			close_it = false
		};
		if( jQuery( event.target ).parents( '.mce_' + otw_overlay_shortcode_component.tinymce_button_key ).size() ){
			close_it = false;
		};
			...
			if( matches = jQuery( this ).attr( 'class' ).match( /^otw\-shortcode\-dropdown\-action\-([a-zA-Z0-9_\-]+)$/ ) ){
				...
				switch( matches[1] ){
					...
					case 'close':
						...
						break;
					default:
							jQuery.get( oSelf.wp_url + '.php?action=otw_overlay_shortcode_editor_dialog&shortcode=' + matches[1],function(b){
								...
								otw_shortcode_editor.preview = oSelf.shortcodes[ matches[1] ].object.preview;
								...
								tb_show( oSelf.get_label( 'Insert' ) + ' OTW ' + oSelf.shortcodes[ matches[1] ].title, "#TB_inline?width="+f+"&height="+b+"&inlineId=otw-dialog" );
...
otw_shortcode_editor_object.prototype.live_reload = function(){
	...
	jQuery.post( this.wp_url + '.php?action=otw_overlay_shortcode_live_reload&shortcode=' + this.shortcode_type , { 'shortcode': s_code, 'post': post_id }, function( response ){
...
otw_shortcode_editor_object.prototype.live_preview = function(){
	...
	jQuery.post( this.wp_url + '.php?action=otw_overlay_shortcode_live_preview&shortcode=' + this.shortcode_type , { 'shortcode': s_code, 'post': post_id }, function( response ){
...
otw_shortcode_editor_object.prototype.get_code = function(){
	...
	jQuery.post( this.wp_url + '.php?action=otw_overlay_shortcode_get_code&shortcode=' + this.shortcode_type , this.code, function( response ){
...
function otw_shortcode_editor_element( element_type, element ){
	...
	element.on( 'change',  function(){
		...
		oSelf.current_value = element.val();
...
var otw_overlay_shortcode_component = null;
