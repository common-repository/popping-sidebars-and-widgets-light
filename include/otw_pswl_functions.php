...
if( !function_exists( 'otw_pswl_init' ) ){
	...
	function otw_pswl_init(){
		...
		if( is_array( $custom_post_types ) ){
			foreach( $custom_post_types as $c_key => $c_cust ){
				...
				if( otw_pswl_installed_plugin( 'bbpress' ) && $c_key == 'reply' ){
					...
					$wp_pswl_int_items[ 'cpt_'. $c_cust->name ] = array( array(), $c_cust->label, esc_html__( 'All ', 'otw_pswl' ).$c_cust->labels->name );
				}
			}
		}
			...
			foreach( $custom_taxonomies as $c_cust ){
				...
				foreach( $c_cust->object_type as $c_object ){
					...
					if( $c_object_info = get_post_type_object( $c_object ) ){
						$wp_pswl_int_items[ $c_object.'_in_ctx_'. $c_cust->name ] = array( array(), esc_html__( 'All', 'otw_pswl' ).' '.$c_object_info->labels->name.' '.__( 'from taxonomy', 'otw_pswl' ).' '.$c_cust->label, esc_html__( 'All', 'otw_pswl' ).' '.$c_object_info->labels->name.' '.__( 'from taxonomy', 'otw_pswl' ).' '.$c_cust->label );
					}
				}
		...
		$otw_pswl_grid_manager_object->text_info = esc_html__( 'Add some rows and columns in the rows. Then you will be able to add sidebars in the columns. Once you build your layout you can save it so you can use it for another page.', 'otw_pswl' );
		...
		$otw_pswl_shortcode_object->shortcodes['sidebars'] = array( 'title' => esc_html__('Sidebars', 'otw_pswl'),'enabled' => true,'children' => false,'order' => 132,'parent' => false, 'path' => dirname( __FILE__ ).'/otw_components/otw_overlay_shortcode/', 'url' => $otw_pswl_plugin_url.'/include/otw_components/otw_overlay_shortcode/' );
		...
		if( otw_init_filesystem() ){
			...
			if( $wp_filesystem->is_file( plugin_dir_path( __FILE__ ).'otw_labels/otw_pswl_form_object.labels.php' ) ){
		...
		$otw_pswl_overlay_object->views['full_bar'] = 'views/pswl/full_bar';
		$otw_pswl_overlay_object->views['side_box'] = 'views/pswl/side_box';
		$otw_pswl_overlay_object->views['lightbox'] = 'views/pswl/lightbox';
		...
		$otw_pswl_factory_object->add_plugin( $otw_pswl_plugin_id, dirname( dirname( __FILE__ ) ).'/otw_popping_sidebars_and_widgets.php', array( 'menu_parent' => 'otw-pswl', 'lc_name' => esc_html__( 'License Manager', 'otw_pswl' ), 'menu_key' => 'otw-pswl' ) );
...
if( !function_exists( 'otw_pswl_init_admin_menu' ) ){
	...
	function otw_pswl_init_admin_menu(){
		...
		add_menu_page(__('Popping Sidebars and Widgets Light', 'otw_pswl'), esc_html__('Popping Sidebars and Widgets Light', 'otw_pswl'), 'manage_options', 'otw-pswl', 'otw_pswl_list', $otw_pswl_plugin_url.'/images/otw-menu-icon.png');
		add_submenu_page( 'otw-pswl', esc_html__('All Overlays', 'otw_pswl'), esc_html__('All Overlays', 'otw_pswl'), 'manage_options', 'otw-pswl', 'otw_pswl_list' );
		add_submenu_page( 'otw-pswl', esc_html__('Manage Overlay', 'otw_pswl'), esc_html__('Add Overlay', 'otw_pswl'), 'manage_options', 'otw-pswl-manage', 'otw_pswl_manage' );
		add_submenu_page( 'otw-pswl', esc_html__('All Sidebars', 'otw_pswl'), esc_html__('All Sidebars', 'otw_pswl'), 'manage_options', 'otw-pswl-sidebars-list', 'otw_pswl_sidebars_list' );
		add_submenu_page( 'otw-pswl', esc_html__('Manage Sidebar', 'otw_pswl'), esc_html__('Add Sidebar', 'otw_pswl'), 'manage_options', 'otw-pswl-sidebars-manage', 'otw_pswl_sidebars_manage' );
		add_submenu_page( __FILE__, esc_html__('Overlay action', 'otw_pswl'), esc_html__('Overlay action', 'otw_pswl'), 'manage_options', 'otw-pswl-action', 'otw_pswl_action' );
		add_submenu_page( __FILE__, esc_html__('Sidebar action', 'otw_pswl'), esc_html__('Sidebar action', 'otw_pswl'), 'manage_options', 'otw-pswl-sidebars-action', 'otw_pswl_sidebars_action' );
		add_submenu_page( 'otw-pswl', esc_html__( 'Plugin Options', 'otw_pswl'), esc_html__('Plugin Options', 'otw_pswl'), 'manage_options', 'otw-pswl-options', 'otw_pswl_plugin_options' );
	}
}
...
if( !function_exists( 'otw_pswl_get_item_sort_options' ) ){
	function otw_pswl_get_item_sort_options( $item_type ){
		...
		switch( $item_type ){
			...
			case 'page':
			case 'post':
					$sort_options['a_z'] = esc_html__( 'Alphabetically: A-Z', 'otw_pswl' );
					$sort_options['z_a'] = esc_html__( 'Alphabetically: Z-A', 'otw_pswl' );
					$sort_options['date_latest'] = esc_html__( 'Latest created', 'otw_pswl' );
					$sort_options['date_oldest'] = esc_html__( 'Oldest created', 'otw_pswl' );
					$sort_options['modified_latest'] = esc_html__( 'Latest Modified', 'otw_pswl' );
					$sort_options['modified_oldest'] = esc_html__( 'Oldest Modified', 'otw_pswl' );
				break;
	...
	function otw_pswl_get_filtered_items( $type, $filter, $overlay_id, $displayed_items = 20, $id_in_list = array(), $id_not_in_list = array(), $show = 'all', $order = 'a_z', $current_page = 0 ){
		...
		switch( $type )
		{
			...
			case 'userroles':
					...
					$all_items['notlogged'] = esc_html__( 'Not Logged in', 'otw_pswl' );
					...
					foreach( $all_items as $u_role_code => $u_role_name ){
						...
						if( $u_role_code != 'notlogged' ){
							$item->name = esc_html__( 'Logged in as ', 'otw_pswl' ).$u_role_name;
						}else{
			...
			case 'bbp_page':
					if( otw_pswl_installed_plugin( 'bbpress' ) ){
						...
						$bbp_pages[] = array( 'id' => 'forums', 'name' => esc_html__( 'Forums', 'otw_pswl' ) );
						$bbp_pages[] = array( 'id' => 'noreplies', 'name' => esc_html__( 'Topics no reply', 'otw_pswl' ) );
						$bbp_pages[] = array( 'id' => 'mostpopular', 'name' => esc_html__( 'Topics popular', 'otw_pswl' ) );
						$bbp_pages[] = array( 'id' => 'search', 'name' => esc_html__( 'Search', 'otw_pswl' ) );
						$bbp_pages[] = array( 'id' => 'singleuser', 'name' => esc_html__( 'User pages', 'otw_pswl' ) );
...
if( !function_exists( 'otw_pswl_get_wp_items' ) ){
	function otw_pswl_get_wp_items( $item_type ){
		switch( $item_type ){
			...
			case 'archive':
					$archive_types = array();
					$a_types = array( 'daily' => esc_html__('Daily', 'otw_pswl'), 'monthly' => esc_html__('Monthly', 'otw_pswl'), 'yearly' => esc_html__('Yearly', 'otw_pswl') );
			...
			case 'templatehierarchy':
					$h_types = array();
					$a_types = array(
							'home'        =>    esc_html__('Home', 'otw_pswl'),
							'front'       =>    esc_html__('Front Page', 'otw_pswl'),
							'404'         =>    esc_html__('Error 404 Page', 'otw_pswl'),
							'search'      =>    esc_html__('Search', 'otw_pswl'),
							'date'        =>    esc_html__('Date', 'otw_pswl'),
							'author'      =>    esc_html__('Author', 'otw_pswl'),
							'category'    =>    esc_html__('Category', 'otw_pswl'),
							'tag'         =>    esc_html__('Tag', 'otw_pswl'),
							'taxonomy'    =>    esc_html__('Taxonomy', 'otw_pswl'),
							'archive'     =>    esc_html__('Archive', 'otw_pswl'),
							'single'      =>    esc_html__('Singular', 'otw_pswl'),
							'attachment'  =>    esc_html__('Attachment', 'otw_pswl'),
							'page'        =>    esc_html__('Page', 'otw_pswl')
						);
