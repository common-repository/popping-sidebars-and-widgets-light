<?php
	...
	$page_title = esc_html__( 'Create New overlay', 'otw_pswl' );
	...
	if( otw_get('overlay',false) ){
		...
		if( is_array( $otw_overlays ) && isset( $otw_overlays[ $otw_overlay_id ] ) ){
			...
			$page_title = esc_html__( 'Edit overlay', 'otw_pswl' );
		}
	}
...
<div class="wrap">
	...
	<h2>
		<?php echo $page_title; ?>
		<a class="preview button" href="admin.php?page=otw-pswl"><?php esc_html_e('Back To Available overlays', 'otw_pswl')?></a>
	</h2>
	<?php include_once( 'otw_pswl_help.php' );?>
	<div class="updated <?= $info_box_class?>">
		<div class="otw_info_box_controls">
			<a href="javascript:;" class="otw_open_info_box"><?php esc_html_e( 'info', 'otw_pswl' )?></a>
			<a href="javascript:;" class="otw_close_info_box"><?php esc_html_e( 'x', 'otw_pswl' )?></a>
		</div>
		<div class="otw_info_box_content">
			<p><?php esc_html_e( 'Wondering how this works?', 'otw_pswl' );?></p>
			<ul>
				<li><?php esc_html_e( '1. Choose the Overlay Type', 'otw_pswl' );?></li>
				<li><?php echo otw_esc_text( esc_html( '2. Set it up in <a href="javascript:;" class="otw_scrollto" rel="otw_options_main">Overlay Options</a> bellow', 'otw_pswl' ), 'rcont' );?></li>
				<li><?php echo otw_esc_text( esc_html( '3. If you need to style it add a custom css class in <a href="javascript:;" class="otw_scrollto" rel="otw_options_custom">Custom Styling Options</a> bellow', 'otw_pswl' ), 'rcont' );?></li>
				<li><?php echo otw_esc_text( esc_html( '4. Go down to the <a href="javascript:;" rel="otw_grid_manager_content" class="otw_scrollto">OTW Grid Manager Light</a> to create a layout and insert sidebars in your overlay. The layout is the skeleton that holds the sidebars and makes sure everything is well arranged.', 'otw_pswl' ), 'rcont' );?></li>
				<li><?php echo otw_esc_text( esc_html( '5. If you need to have custom sidebars you can create as many sidebars as you need from <a href="admin.php?page=otw-pswl-sidebars-manage">Add Sidebar</a> in the left menu of the plugin.', 'otw_pswl' ), 'rcont' );?></li>
			</ul>
		</div>
	</div>
				...
				<div id="post-body-content">
					<div id="otw-col-left">
						<div class="form-field form-required">
							<label for="title"><?php esc_html_e( 'Overlay Title', 'otw_pswl' );?></label>
							<input type="text" id="title" value="<?php echo esc_attr( $otw_overlay_values['title'] )?>" tabindex="1" size="30" name="title"/>
							<p><?php esc_html_e( 'The name is how it appears on your admin panel.', 'otw_pswl' );?></p>
						</div>
						<div class="form-field">
							<label for="status"><?php esc_html_e( 'Status', 'otw_pswl' );?></label>
							<select id="status" tabindex="2" style="width: 170px;" name="status">
								<option value="active"<?php if( $otw_overlay_values['status'] == 'active' ){ echo ' selected="selected" ';}?>><?php esc_html_e( 'Active', 'otw_pswl' )?></option>
								<option value="inactive"<?php if( $otw_overlay_values['status'] == 'inactive' ){ echo ' selected="selected" ';}?>><?php esc_html_e( 'Inactive', 'otw_pswl' )?></option>
							</select>
						</div>
						<div class="form-field form-required">
							<label for="type"><?php esc_html_e( 'Overlay Type', 'otw_pswl' );?></label>
							...
							<p><?php esc_html_e( 'Choose the type of your overlay.', 'otw_pswl' );?></p>
						</div>
						<?php foreach( $otw_pswl_overlay_object->overlay_types as $overlay_type => $overlay_data ){?>
										...
										<?php if( in_array( $option_name, array( 'main', 'custom' ) ) ){?>
											<div class="meta-box-sortables otw_options_<?= $option_name?>">
												<div data-type="<?= $option_name?>" class="postbox<?php echo ( $page_settings['metabox_'.$option_name ] == 'closed'  )?' closed':''?>">
													<div title="<?php esc_html_e('Click to toggle', 'otw_pswl')?>" class="handlediv sitem_toggle"><br /></div>
						...
						<div>
							<p class="submit">
								<input type="submit" value="<?php esc_html_e( 'Save Overlay', 'otw_pswl') ?>" name="submit" class="button button-primary button-large"/>
							</p>
						</div>
					</div>
					<div id="otw-col-right" class="otw_pswl_<?php echo esc_attr( $otw_overlay_id )?>">
						
					</div>
					<div id="<?php echo esc_attr( $otw_pswl_grid_manager_object->meta_name );?>">
						<div class="meta-box-sortables">
							<div class="postbox">
								<h3 class="hndle sitem_header"><span><?php esc_html_e( 'OTW Grid Manager Light', 'otw_pswl' )?></span></h3>
								<div class="inside">
